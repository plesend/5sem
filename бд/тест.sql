...
-------
--1
create or replace procedure COUNTSUM (customer_name in VARCHAR2) 
is  v_customer_num CUSTOMERS.CUST_NUM%TYPE;
    v_total number := 0;
    
    cursor c_orders is
        select order_num, order_date, amount from orders where cust = v_customer_num;
        
    v_order c_orders%rowtype;
begin 
    select cust_num into v_customer_num from customers where company = customer_name;
    
    DBMS_OUTPUT.PUT_LINE ('Orders of ' || customer_name || ': ');
    ...
    loop
        ...
        DBMS_OUTPUT.PUT_LINE('Order number: ' || v_order.order_num || ' ' ||
                'Order date: ' || v_order.order_date || ' ' || 'Price: ' || v_order.amount);
        
        v_total := v_total + v_order.amount;
    end loop;
    close c_orders;
    
    DBMS_OUTPUT.PUT_LINE ('Price summary: ' || v_total);
    
exception
    when no_data_found then
        DBMS_OUTPUT.PUT_LINE ('Nothing here');
    when others then
        DBMS_OUTPUT.PUT_LINE ('PROBLEMO');
end;
/

SET SERVEROUTPUT ON
    
begin COUNTSUM('Ace International'); end;

--2
create or replace function countorder (company_inp varchar2, startdate date, enddate date) return number is 
    v_count number;
begin
    select count(*) into v_count from orders o join  customers c on o.cust = c.cust_num 
    where (c.company = company_inp) and (order_date between startdate and enddate);
...
--create or replace procedure tovarlist(p_company varchar2) is    
--    cursor c_tovars is 
--        select c.company, p.description, sum(o.amount) as total_sm
--        from orders o 
--        join customers c on o.cust = c.cust_num
--        join products p on o.mfr =p.mfr_id
--        where c.company = p_company
--        group by p.description, c.company
--        order by total_sm asc;
        
--    v_tovars c_tovars%rowtype;
--begin 
--    DBMS_OUTPUT.PUT_LINE('Company: ' || company);
--    open c_tovars;
--    loop
--    fetch c_tovars into v_tovars;
--    exit when c_tovars%notfound;
--    DBMS_OUTPUT.PUT_LINE('Product: ' || v_tovars.description || ' ' || 'Summary: ' || v_tovars.total_sm);
--    end loop;
--    close c_tovars;
--EXCEPTION 
--when others then
--DBMS_OUTPUT.PUT_LINE('PROBLEMO' || SQLERRM);
--end;


begin tovarlist('Ace International'); end;

--4 

create sequence customers_seq_test
    start with 3000
    increment by 1
    nocache
    nocycle;
...
create or replace function add_customer
    ...
    p_COMPANY  VARCHAR2,
    p_CUST_REP INTEGER,
    p_CREDIT_LIMIT DECIMAL) RETURN NUMBER IS
        v_cust_num customers.cust_num%type;
begin
    v_cust_num := customers_seq_test.nextval;
    insert into customers values (v_cust_num, p_company, p_cust_rep, p_credit_limit);
    return v_cust_num;
...
begin 
    v_new_id := add_customer('cool company', 105, 50000);
    dbms_output.put_line('new: ' || v_new_id);
end;
    
SELECT * FROM customers WHERE cust_num >= 3000;

--5
create or replace procedure custlist(startdate date, enddate date) is
    cursor c_cust_amount is
        select c.company, c.cust_num, sum(o.amount) as total_sm
        from customers c 
        join orders o on c.cust_num = o.cust
        where o.order_date between startdate and enddate
        group by c.company, c.cust_num
        order by total_sm desc;
    v_cust_amount c_cust_amount%rowtype;
begin
    open c_cust_amount;
    loop 
    fetch c_cust_amount into v_cust_amount;
    exit when c_cust_amount%notfound;
    dbms_output.put_line('company: ' || v_cust_amount.company || ' ' ||
        'cust id: ' || v_cust_amount.cust_num || ' ' || 'total: ' || v_cust_amount.total_sm || ' ');
    end loop;
    close c_cust_amount;
exception 
    when others then 
...
begin custlist(date '2000-05-10', date '2016-03-01');end;


--6
create or replace function custcount(p_good varchar2) return number is
    p_count number;
begin
...
declare 
    v_return number;
begin
    v_return:= custcount('Ratchet Link');
    dbms_output.put_line('Quantity: ' || v_return);
end;

--8
create or replace procedure plus10(p_description varchar2) is        
begin
    update products 
    set price = price *1.1
...
SET SERVEROUTPUT ON
create or replace procedure proc(p_customer varchar2) is
    v_max_price products.price%type;
    
    cursor c_tovarlist is
        select p.description, p.price
        from products p
        where p.price > v_max_price;
                
    v_tovarlist c_tovarlist%rowtype;
begin
    select max(p1.price)
    into v_max_price 
    from products p1
    join orders o1 on o1.product = p1.product_id
    join customers c1 on c1.cust_num = o1.cust
    where c1.company = p_customer;

    dbms_output.put_line('Goods list: ');
    open c_tovarlist;
    loop
    fetch c_tovarlist into v_tovarlist;
    exit when c_tovarlist%notfound;
    dbms_output.put_line(v_tovarlist.description || ' ' || v_tovarlist.price || ' ');
    end loop;
    close c_tovarlist;
exception
    when no_data_found then 
...
begin proc('JCP Inc.'); end;

--======================

create or replace procedure minus10(p_description varchar2) is 
...
begin
    ...
    dbms_output.put_line(p_description || ' ' || v_new_price);
...
begin minus10('Ratchet Link'); end;
...
create or replace procedure myfunc is 
    ...
    cursor c_offices is 
        ...
        from offices o 
        where not exists (select 1 from salesreps s where s.rep_office = o.office);
        
    v_offices c_offices%rowtype;
begin    
    select count(*) into v_count from offices o 
        where not exists (select 1 from salesreps s where s.rep_office = o.office);
        
    if v_count > 0 then 
        dbms_output.put_line('offices without workers: ');
        
        open c_offices;
        loop
        fetch c_offices into v_offices;
        exit when c_offices%notfound;
        dbms_output.put_line('office numba: ' || v_offices.office || 'office cie: ' || v_offices.city);
        end loop;
        close c_offices;
    
    else 
        for r in(
        select o.office, o.city, count(s.empl_num) as empl_num
        from offices o 
        join salesreps s on s.rep_office = o.office
        group by o.office, o.city ) loop
            dbms_output.put_line('office numba: ' || r.office || 'office cie: ' || r.city);
        end loop;
...
create or replace function myfunc1(p_region varchar2) return number is
    v_count number;
begin
    select count(*) 
    into v_count
    ...
    join salesreps s on s.empl_num = c.cust_rep
    join offices ofc on ofc.office = rep_office
    where p_region = ofc.region;
