...
create or replace procedure myproc1(p_office varchar2, startdate date, enddate date) is
    v_sum DECIMAL(9,2);

    cursor c_orders is
        select o.order_num, o.order_date, o.amount
        from orders o
        join customers c on c.cust_rep = o.rep
        join salesreps s on c.cust_rep = s.empl_num
        join offices ofc on s.rep_office = ofc.office
        where (ofc.office = p_office) and (o.order_date between startdate and enddate)
        group by o.order_num, o.order_date, o.amount
        order by o.amount desc;
        
    v_orders c_orders%rowtype;

begin

    if (enddate < startdate) then
        dbms_output.put_line('error in dates');
    else
    ...
    open c_orders;
    loop
    fetch c_orders into v_orders;
    exit when c_orders%notfound;
    dbms_output.put_line('Order num: ' || v_orders.order_num || ' ' || 'Order date: ' ||  
        v_orders.order_date || ' ' || 'Sum: ' || v_orders.amount);
    end loop;
    close c_orders;
...
begin myproc1(22, date '2000-01-01', date '1999-01-01');end;
