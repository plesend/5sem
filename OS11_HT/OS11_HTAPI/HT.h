#pragma once
#include <windows.h>  // для HANDLE, BOOL, LPVOID
...
// ---------------------------------------------------------------------------
// Формат на диске / утилиты
// ---------------------------------------------------------------------------
#pragma pack(push, 1)
struct HTAPI_API DiskHeader {
	uint magic;              // magic для распознавания формата
	int capacity;            // количество слотов
	int secSnapshotInterval; // интервал снапшота в секундах
	int maxKeyLength;        // макс длина ключа
	int maxPayloadLength;    // макс длина payload
	time_t lastsnaptime;         // время последнего снапшота
	char reserved[256];          // резерв
};
#pragma pack(pop)

struct HTAPI_API HTHANDLE    // блок управления HT
{
	HTHANDLE();
...
struct HTAPI_API Element   // элемент 
{
	Element();
...
extern "C" {
	...
	HTAPI_API HTHANDLE* Create   //  создать HT             
	(
	...
	HTAPI_API HTHANDLE* Open     //  открыть HT             
	(
		const char    FileName[512]         // имя файла 
	); 	// != NULL успешное завершение  

	HTAPI_API BOOL Snap         // выполнить Snapshot
	(
		const HTHANDLE* hthandle           // управление HT (File, FileMapping)
	);


	HTAPI_API BOOL Close        // Snap и закрыть HT  и  очистить HTHANDLE
	(
		const HTHANDLE* hthandle           // управление HT (File, FileMapping)
	);	//  == TRUE успешное завершение   


	HTAPI_API BOOL Insert      // добавить элемент в хранилище
	(
	...
	HTAPI_API BOOL Delete      // удалить элемент в хранилище
	(
		const HTHANDLE* hthandle,            // управление HT (ключ)
		const Element* element              // элемент 
	);	//  == TRUE успешное завершение 

	HTAPI_API Element* Get     //  читать элемент в хранилище
	(
		const HTHANDLE* hthandle,            // управление HT
		const Element* element              // элемент 
	); 	//  != NULL успешное завершение 
	...
	HTAPI_API BOOL Update     //  именить элемент в хранилище
	(
		const HTHANDLE* hthandle,            // управление HT
	...
	HTAPI_API const char* HTGetLastError  // получить сообщение о последней ошибке
	(
		HTHANDLE* ht                         // управление HT
	);

	HTAPI_API void HTPrint                               // распечатать элемент 
	(
		const Element* element              // элемент 
	);
}
