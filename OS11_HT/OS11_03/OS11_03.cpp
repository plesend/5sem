...
int main(int argc, char** argv)
{
    ...
    if (hShutdownEvent == NULL) {
        std::cerr << "Warning: CreateEventA(shutdown) failed: " << GetLastError()
            << " — continuing without global shutdown support." << std::endl;
    }
    else {
    ...
    while (true) {
        ...
        if (hShutdownEvent) {
            ...
            if (s == WAIT_OBJECT_0) {
                cout << "Shutdown event signaled — exiting main loop." << endl;
                break;
            }
        }
        ...
        if (!Delete(h, &e)) {
            cerr << "[DELETE FAIL] key=" << key << ": " << HTGetLastError(h) << endl;
        }
        else {
            cout << "[DELETE] key=" << key << endl;
        }

        // Wait up to 1s for shutdown, otherwise continue
        if (hShutdownEvent) {
            DWORD waitRes = WaitForSingleObject(hShutdownEvent, 1000);
            if (waitRes == WAIT_OBJECT_0) {
                cout << "Shutdown event signaled during wait — exiting." << endl;
                break;
            }
        }
        else {
