...
void* Func(void* arg) {
    ...
    int* tlsBuf = (int*)malloc(MAX_PRIMES_PER_THREAD * sizeof(int));
    int localCount = 0;

    pthread_setspecific(tlsKey, tlsBuf);
    ...
    int* get = (int*)pthread_getspecific(tlsKey);
    if (get) {
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < threadCount; i++) {
        ...
        if (pthread_create(&threads[i], NULL, Func, r) != 0) {
